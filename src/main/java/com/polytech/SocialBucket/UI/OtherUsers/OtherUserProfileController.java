...
public class OtherUserProfileController {
    ...
    @FXML
    private void toggleFollow(ActionEvent event) {
        if (isFollowing) {
            userFacade.unfollowUser(userFacade.getCurrentUser().getId(),
                    userFacade.getCurrentViewedUser().getId());
            ...
            followerLabel.setText("Followers: " + userFacade.getCurrentViewedUser().getNbFollowers());
        } else {
            userFacade.followUser(userFacade.getCurrentUser().getId(),
                    userFacade.getCurrentViewedUser().getId());
            userFacade.getCurrentViewedUser().setNbFollowers(userFacade.getCurrentViewedUser().getNbFollowers() + 1);
            userFacade.getCurrentUser().setNbFollowing(userFacade.getCurrentUser().getNbFollowing() + 1);
            followerLabel.setText("Followers: " + userFacade.getCurrentViewedUser().getNbFollowers());
    ...
    public void initialize() {
        ...
        System.out.println(userFacade.getCurrentViewedUser().getUsername());
        usernameLabel.setText(userFacade.getCurrentViewedUser().getUsername());
        // Set the follower label
        System.out.println(userFacade.getCurrentViewedUser().getNbFollowers());
        followerLabel.setText("Followers: " + userFacade.getCurrentViewedUser().getNbFollowers());
        // Set the following label
        System.out.println(userFacade.getCurrentViewedUser().getNbFollowing());
        followingLabel.setText("Following: " + userFacade.getCurrentViewedUser().getNbFollowing());
        // Set the follow button
        this.isFollowing = userFacade.isFollowing(userFacade.getCurrentUser().getId(),
        userFacade.getCurrentViewedUser().getId());
